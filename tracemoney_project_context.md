...
```
User
 â”œâ”€â”€ Web Dashboard (Next.js PWA)
 â””â”€â”€ WhatsApp Bot (Twilio)
          â”‚
          â–¼
     Backend API (Next.js API Routes)
          â”‚
          â”œâ”€â”€ AI Layer (Claude API)
          â”‚     â”œâ”€â”€ Transaction categorisation
          â”‚     â”œâ”€â”€ NL query answering
          â”‚     â””â”€â”€ Monthly insight generation
          â”‚
          â”œâ”€â”€ Data Layer (Supabase / PostgreSQL)
          â”‚     â”œâ”€â”€ transactions
          â”‚     â”œâ”€â”€ categories (12 seeded)
          â”‚     â”œâ”€â”€ payment\\\_sources
          â”‚     â”œâ”€â”€ budgets
          â”‚     â”œâ”€â”€ insights (cached AI output)
          â”‚     â””â”€â”€ whatsapp\\\_sessions + messages
          â”‚
          â””â”€â”€ Data Sources
                â”œâ”€â”€ RBI Account Aggregator API (consent-based)
                â””â”€â”€ SMS parser (UPI alert fallback)
```
...
* Subscription tracker â€” detects recurring charges, projects next charge date, calculates monthly burn
* Category deep dive â€” weekly trend line, top merchants, average transaction value
* Budget vs actual progress bars with overspend alerts

### 4\. Natural Language Query Interface

* Conversational chat UI on the dashboard â€” same backend as the WhatsApp bot
...
* Streaming response â€” tokens rendered progressively in the UI
...
* Webhook receives inbound messages â†’ same `/api/ask` route as dashboard chat
* Conversation history stored per phone number in `whatsapp\\\_messages` table
* No separate model â€” same Claude API call, same context injection

### 6\. Security

* RBI Account Aggregator framework â€” user explicitly consents to each data fetch, revocable at any time
* Row Level Security (RLS) enabled on all Supabase tables â€” users can only access their own data
* JWT-based auth with short expiry
* No bank credentials stored at any point
* Raw transaction data never forwarded to third-party APIs â€” summarised on backend before Claude call
* Demo mode uses seeded data â€” no real financial data exposed in presentations
...
* `profiles` â€” extends Supabase auth, auto-created on signup via DB trigger
* `categories` â€” 12 system categories with icon, colour, seeded on schema creation
* `payment\\\_sources` â€” UPI, card, wallet, netbanking per user
* `transactions` â€” core table; stores amount, merchant, raw description, category, recurring flag, AI categorisation flag
* `budgets` â€” monthly spend limits per category
* `insights` â€” cached AI-generated monthly summaries (avoids redundant API calls)
* `whatsapp\\\_sessions` â€” maps phone number to user
* `whatsapp\\\_messages` â€” full conversation history per session
...
|Page|Purpose|
|-|-|
|Landing|Marketing page â€” problem, product showcase, features, security trust|
|Dashboard|Home â€” summary cards, category donut chart, recent transactions, AI insight, subscriptions, budget progress|
|Transactions|Full ledger â€” search, filter by category/source/type/date, edit category, add notes, export CSV|
...
|Ask TraceMoney|Conversational AI chat â€” same engine as WhatsApp bot|

\---

## DS / ML Highlights

* **Prompt engineering** â€” structured output extraction from noisy financial text using chain-of-thought prompting and JSON mode
* **Time-series analysis** â€” month-over-month spend aggregation, weekly trend detection, anomaly flagging for unusual category spikes
* **Retrieval-augmented context** â€” last 90 days of transaction data summarised and injected as context for NL query answering
* **Recurring charge detection** â€” heuristic + LLM hybrid to identify subscription patterns from transaction history
* **Spend forecasting** â€” linear projection of monthly spend based on daily average (days elapsed / days in month)
...
* Monthly salary credit of â‚¹70,000 on the 1st
